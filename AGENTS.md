...
- **Folder count badge**: Each directory item in `explorer_browse` response includes `file_count` and `dir_count` fields. Backend uses batched `GROUP BY` queries for DB-sourced directories and `os.listdir()` for filesystem-scanned upload dirs. Frontend renders a small count badge (`.explorer__tile-count`) at top-left of the folder thumbnail in grid view, and a descriptive tooltip (`title` attribute) on hover. List view shows the count text in `.explorer__tile-meta`.
