...
- **Admin Tasks PIN gate**: `adminPin` stored in IndexedDB via `getPref("adminPin")`. Validated purely client-side (compare entered PIN against stored value). On page load, if PIN exists, section is locked until correct PIN entered. Lock/unlock state is session-only (resets on page refresh). Change PIN flow: enter current PIN → enter new PIN. Setting a PIN when none exists: enter new PIN → stored in IndexedDB. Admin task rows are disabled (`.settings__row--locked`, opacity 0.4) when locked.
